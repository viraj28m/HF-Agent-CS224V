            ...
            while not week_complete:
                ...
                try:
                    ...
                    if "WEEK_COMPLETE" in response:
                        ...
                        if ("MEDICATION TITRATION PLAN" in response or 
                            "titration" in response.lower() or
                            ("physician" in response.lower() and "recommend" in response.lower()) or
                            ("hold" in response.lower() and "increase" in response.lower()) or
                            ("continue" in response.lower() and "medication" in response.lower())):
                            week_complete = True
                            console.print(f"\n[bold green]✅ Week {week} completed with titration decisions! Moving to next week...[/bold green]")
...
def automated(patient_id: str, weeks: int, adherence_pattern: str, 
    ...
    for week in range(1, weeks + 1):
        ...
        try:
            initial_prompt = f"""Begin week {week} check-in for {patient_state.patient_name}. Follow your conversation protocol and begin the check-in now."""
            ...
            for exchange in range(max_exchanges):
                ...
                if "WEEK_COMPLETE" in hf_message:
                    ...
                    if ("MEDICATION TITRATION PLAN" in hf_message or 
                        "titration" in hf_message.lower() or
                        ("physician" in hf_message.lower() and "recommend" in hf_message.lower()) or
                        ("hold" in hf_message.lower() and "increase" in hf_message.lower()) or
                        ("continue" in hf_message.lower() and "medication" in hf_message.lower())):
